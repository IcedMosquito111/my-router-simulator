...
```powershell
.\.venv\Scripts\python.exe -m pytest              # 全部 65 项，约 11 秒
...
| `tests/test_ws_integration.py` | 真实 uvicorn + WebSocket：非法命令不断连、49 张路由表推送、`/api/metrics`、拓扑导入导出往返 |
...
| 10 | 无 README/依赖锁定/测试；`CORS: allow_origins=["*"] + allow_credentials=True` 矛盾 | 补齐文档与依赖、加入 65 项测试、按通配来源自动关闭凭证、配置全部环境变量化 |
